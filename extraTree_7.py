...
from sklearn.ensemble import ExtraTreesClassifier as algo
...
# if model exist load it , otherwise build it .. 

model_name = './model/extraTree__fea_ext_ka_1200.pkl'
model_exist = False 
...
#pred_valid_y = est.predict_proba(X_valid)[:,1]

#testEstimator(est , y_valid , pred_valid_y)
preds = est.predict_proba(test)[:,1]
create_submission(preds, "")

if not model_exist:
    print("Saving the Model")
    from sklearn.externals import joblib
    joblib.dump(est, './model/extraTree__fea_ext_ka_1200.pkl')
